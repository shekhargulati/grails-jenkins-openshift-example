		...
		<div id="list-todo" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="submittedBy" title="${message(code: 'todo.submittedBy.label', default: 'Submitted By')}" />
					
						<g:sortableColumn property="task" title="${message(code: 'todo.task.label', default: 'Task')}" />
				...
				<g:each in="${todoInstanceList}" status="i" var="todoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${todoInstance.id}">${fieldValue(bean: todoInstance, field: "description")}</g:link></td>
					
						<td>${fieldValue(bean: todoInstance, field: "submittedBy")}</td>
					
						<td>${fieldValue(bean: todoInstance, field: "task")}</td>
